...
-- 本地全新建库脚本（删库 → 重建全部表）
-- =============================================================================
-- 内容 = d:\表结构.sql（7 张共用表）+ 子系统5 追加表（17 张）+ user 扩展 2 列
-- 共 24 张表。库名 overseas_artifacts（与 application.yml 一致）。
--
-- 【本地】一条命令重建干净环境：
...
--   子系统5 新增 17 张：role_definitions, permission_definitions, role_permission_assignments,
--     admin_users, admin_role_permission_audit, user_behaviors, user_permission_audit,
--     review_contents, sensitive_words, review_strategy_config,
